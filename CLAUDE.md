...
- The `messages` table is **not** defined in `supabase/schema.sql` — it was created via the Supabase dashboard/MCP. If you need its shape, query Supabase directly rather than guessing from `schema.sql`.

### Trade history — localStorage, not Supabase

This is the trap: the marquee feature does **not** use the database.

- `src/components/sections/trade-history.tsx` + `src/lib/use-trades.ts` persist to `localStorage` key `stock-trades-v1` via `useSyncExternalStore`. Server never sees the data.
- `supabase/schema.sql` defines a `public.trades` table with permissive RLS, but nothing in `src/` reads or writes it. Treat the SQL as aspirational scaffolding until a migration to Supabase is actually wired up.
